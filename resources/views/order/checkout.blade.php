...
<div class="order-checkout-container">
  ...
  <form class="order-checkout-form" action="{{ route('order.confirm') }}" method="POST">
    @csrf
    <div class="order-checkout-section">
      ...
      <div class="zip-inputs">
        <input type="text" id="zip1" name="zip1" maxlength="3" placeholder="000" required>
        <span>-</span>
        <input type="text" id="zip2" name="zip2" maxlength="4" placeholder="0000" required>
      </div>
      ...
      <div class="phone-inputs">
        <input type="text" id="phone1" name="phone1" maxlength="3" placeholder="090" required>
        <span>-</span>
    ...
    <div class="payment-method-section">
      <label class="payment-label">
        <input type="radio" name="payment_option" value="credit">
        クレジットカード
      </label>
      <div id="credit-card-details">
        <label for="cardholder-name">カード名義人 (ローマ字):</label>
        <input type="text" id="cardholder-name" name="cardholder_name" placeholder="例: TARO YAMADA" required>

        <label for="card-number">カード番号:</label>
        <input type="text" id="card-number" name="card_number" placeholder="半角数字のみ (ハイフンなし)">
        <label for="expiration-date">有効期限:</label>
        <div class="expiration-date">
          <input type="text" id="expiration-month" name="expiration_month" placeholder="月">
          <span>/</span>
          <input type="text" id="expiration-year" name="expiration_year" placeholder="年">
        </div>
        <label for="security-code">セキュリティコード:</label>
        <input type="text" id="security-code" name="security_code" placeholder="半角数字のみ">
      </div>
      <label class="payment-label">
    ...
    <button type="submit" class="confirm-order-button">注文を確定する</button>

    <script src="{{ asset('js/order-checkout.js') }}"></script>
